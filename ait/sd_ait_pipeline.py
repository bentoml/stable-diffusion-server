...
class SDAITPipeline(StableDiffusionPipeline):
    def __init__(
        ...
        # TODO: update workdir to your own path
        workdir = 'tmp/'
        self.clip_ait = self.init_ait_module(
            model_name="CLIPTextModel",
            workdir=workdir
        )
        self.unet_ait = self.init_ait_module(
            model_name="UNet2DConditionModel",
            workdir=workdir
        )
        self.vae_ait = self.init_ait_module(
            model_name="AutoencoderModel",
            workdir=workdir
        )

    def init_ait_module(self, model_name, workdir):
        return Model(os.path.join(workdir, model_name, 'test.so'))

    def unet_infernence(self, latent_model_input, timesteps, encoder_hidden_states):
        exe_module = self.unet_ait
        timesteps = timesteps.expand(latent_model_input.shape[0])
        inputs = {
            "input0": latent_model_input.permute((0, 2, 3, 1)).contiguous().cuda().half(),
            "input1": '',
            "input2": ''
        }
        ys = []
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # TODO latest diffusers duplicates text embeddings for each generation per prompt.
        ...
        # get the inital random noise unless the usper supplied it

        # Unlike in other pipelines, latents need to be generated in the target device
        # for 1-to-1 results reproducibility with the CompVis implementation.
        # However this current doesn't work in "mps"
        ...
        for i, t in enumerate(self.progress_bar(self.scheduler.timestemps)):
